  ...
  return (
    ...
    >
      <div className="flex w-full sm:w-auto sm:flex-row sm:items-start sm:gap-3">
        ...
        >
          <ShimmerBar className="h-16 w-full shrink-0 rounded-none" />
          <motion.div
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              <motion.div variants={skeletonBarVariants} custom={1}>
                <ShimmerBar className="h-9 w-28 rounded-2xl" />
              </motion.div>
              <motion.div variants={skeletonBarVariants} custom={2}>
                <ShimmerBar className="h-9 w-36 rounded-2xl" />
              </motion.div>
            </div>
            <motion.div variants={skeletonContainerVariants} className="space-y-2">
            ...
            <motion.div variants={skeletonBarVariants} custom={6}>
              <ShimmerBar className="h-12 w-full rounded-2xl" />
            </motion.div>
            <div className="flex gap-3 border-t border-slate-100 pt-4 dark:border-slate-700">
              <motion.div variants={skeletonBarVariants} custom={7} className="flex-1">
                <ShimmerBar className="h-14 w-full rounded-3xl" />
              </motion.div>
              <motion.div variants={skeletonBarVariants} custom={8}>
                <ShimmerBar className="h-14 w-14 rounded-3xl" />
              </motion.div>
            </div>
          </motion.div>
        </motion.div>
        {showSidePanel && (
          ...
          >
            <ShimmerBar className="h-12 w-full rounded-none" />
      ...
      >
        ...
        >
          ...
          <motion.div variants={skeletonBarVariants} custom={1} className="mt-2">
            <ShimmerBar className="h-4 w-28 rounded-lg" />
          </motion.div>
        </motion.div>
        <div className="flex-1 space-y-4 overflow-hidden bg-slate-50/30 p-6 dark:bg-slate-950/40">
          {Array.from({ length: 2 }).map((_, i) => (
            <motion.div
              ...
              transition={{ type: 'spring', damping: 24, stiffness: 360, delay: 0.06 + i * 0.07 }}
            >
              <ShimmerBar className="h-36 rounded-4xl border border-slate-100 dark:border-slate-800" />
            </motion.div>
          ))}
        </div>
        <div className="shrink-0 border-t border-slate-100 p-6 dark:border-slate-800">
          <ShimmerBar className="mx-auto h-12 w-32 rounded-2xl" />
        </div>
      </motion.div>
    </motion.div>
  );
      ...
      >
        ...
        >
          {Array.from({ length: 7 }).map((_, i) => (
            ...
            >
              <ShimmerBar className="h-3.5 w-3.5 rounded" />
            </motion.div>
          ))}
        </motion.div>
        <motion.div
