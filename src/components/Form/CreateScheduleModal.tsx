...
import {
  X, Calendar as CalendarIcon, AlertCircle, Link as LinkIcon, WifiOff,
  Sparkles, Loader2, CheckCircle2, Plus, ChevronDown, ChevronRight, Trash2,
} from 'lucide-react';
...
type SlotErrors = Partial<Record<'title' | 'startTime' | 'streamerIds', string>>;
...
type EditErrors = Partial<Record<keyof z.infer<typeof editSchema> | 'submit', string>>;
...
function formatSlotPreview(slot: SlotEntry): string {
  ...
  if (slot.startTime) {
    try {
      const d = new Date(slot.startTime.includes('T') ? slot.startTime : slot.startTime + 'T00:00');
      parts.push(format(d, slot.isTimeTBD ? 'M/d' : 'M/d HH:mm', { locale: ko }));
    } catch { /* ignore */ }
  }
  if (slot.selectedStreamerIds.length > 0) parts.push(`${slot.selectedStreamerIds.length}명`);
  return parts.join(' · ');
}
...
export default function ScheduleFormModal({
  ...
  const [participants, setParticipants] = useState<ParticipantEntry[]>(
    initialData?.participants?.map((p) => ({ id: p.id, nation: p.nation ?? '', result: p.result ?? '' })) || [],
  );
  ...
  const [slots, setSlots] = useState<SlotEntry[]>(() => [{ ...createSlot(), key: firstKeyRef.current }]);
  ...
  const toggleStreamer = (id: string) => {
    ...
    if (editErrors.streamerIds) setEditErrors((e) => ({ ...e, streamerIds: undefined }));
  };
  ...
  const handleLiveUrlBlur = useCallback(async () => {
    ...
    try {
      ...
      const filled: string[] = [];
      if (data.title && !title.trim()) { setTitle(data.title); filled.push('제목'); }
      if (data.category && !selectedGameId) {
        const gameId = matchChzzkCategory(data.category);
        if (gameId) {
          const matched = games.find((g) => g.id === gameId);
          if (matched) { setSelectedGameId(matched.id); filled.push(`게임 (${matched.title})`); }
        }
      }
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const parsed = editSchema.safeParse({ title, startTime, streamerIds: selectedStreamers, liveUrl: liveUrl || undefined });
    if (!parsed.success) {
  ...
  const handleSlotLiveUrlBlur = useCallback(async (
    key: string,
    liveUrl: string,
    slotTitle: string,
    slotGameId: string,
    ...
    const trimmed = liveUrl.trim();
    if (!trimmed || !trimmed.includes('chzzk.naver.com')) return;
    setSlots((prev) => prev.map((s) => s.key === key ? { ...s, metaLoading: true, autoFilled: [] } : s));
    try {
      const res = await fetch(`/api/chzzk/live-meta?url=${encodeURIComponent(trimmed)}`);
      if (!res.ok) return;
      const data: AutoFillResult = await res.json();
      ...
      if (data.title && !slotTitle.trim()) { updates.title = data.title; filled.push('제목'); }
      if (data.category && !slotGameId) {
        const gameId = matchChzzkCategory(data.category);
        if (gameId) {
          const matched = games.find((g) => g.id === gameId);
          if (matched) { updates.selectedGameId = matched.id; filled.push(`게임 (${matched.title})`); }
        }
      }
      if (data.matchedStreamerId && !slotStreamerIds.includes(data.matchedStreamerId)) {
        updates.selectedStreamerIds = [...slotStreamerIds, data.matchedStreamerId];
        filled.push(`멤버 (${data.matchedStreamerName})`);
      }
      updates.autoFilled = filled;
      setSlots((prev) => prev.map((s) => s.key === key ? { ...s, ...updates, metaLoading: false } : s));
    } catch {
      setSlots((prev) => prev.map((s) => s.key === key ? { ...s, metaLoading: false } : s));
    }
  }, [games]);
  ...
  const handleBatchSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (failCount === 0) {
      ...
      setBatchSubmitError(`${failCount}개 일정 등록에 실패했습니다. 다시 시도해주세요.`);
    }
    setIsSubmitting(false);
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100">
                {isEdit ? '일정 수정' : createMode === 'batch' && slots.length > 1 ? `일정 ${slots.length}개 등록` : '새 일정 등록'}
              </h3>
            </div>
          </div>
        ...
        {(isEdit || createMode === 'single') ? (
          <>
            ...
            >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (editErrors.liveUrl) setEditErrors((er) => ({ ...er, liveUrl: undefined }));
                    }}
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    {editMetaLoading && <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />}
                    {!editMetaLoading && editAutoFilled.length > 0 && <Sparkles className="w-4 h-4 text-indigo-400" />}
                  </div>
                </div>
                <AnimatePresence>
                  {editAutoFilled.length > 0 && (
                    ...
                    >
                      ...
                      <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400">자동 입력됨: {editAutoFilled.join(' · ')}</p>
                      <button type="button" onClick={() => setEditAutoFilled([])} className="ml-auto text-indigo-300 hover:text-indigo-500 transition-colors">
                        <X className="w-3 h-3" />
                      </button>
                    </motion.div>
                  )}
              ...
              {/* 제목 */}
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase">방송 제목</label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => { setTitle(e.target.value); if (editErrors.title) setEditErrors((er) => ({ ...er, title: undefined })); }}
                ...
                />
                {editErrors.title && <p className="mt-1.5 flex items-center gap-1 text-xs font-bold text-red-500"><AlertCircle className="w-3 h-3 shrink-0" />{editErrors.title}</p>}
              </div>

              {/* 게임 */}
              <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase">플레이 게임 (선택)</label>
                <select
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">시작 시간</label>
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    <input type="checkbox" checked={isTimeTBD} onChange={(e) => setIsTimeTBD(e.target.checked)} className="w-3.5 h-3.5 rounded accent-indigo-600" />
                ...
                <input
                  ...
                  onChange={(e) => { setStartTime(e.target.value); if (editErrors.startTime) setEditErrors((er) => ({ ...er, startTime: undefined })); }}
                ...
                />
                {editErrors.startTime && <p className="mt-1.5 flex items-center gap-1 text-xs font-bold text-red-500"><AlertCircle className="w-3 h-3 shrink-0" />{editErrors.startTime}</p>}
              </div>

              {/* 라이브 강제 종료 (수정 모드 전용) */}
              {isEdit && <div className={`flex items-start gap-3 px-4 py-3.5 rounded-2xl border transition-colors ${isLiveEnded ? 'bg-orange-50 dark:bg-orange-900/15 border-orange-200 dark:border-orange-800' : 'bg-slate-50 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <label className="flex items-center justify-between gap-2 cursor-pointer">
                    <div>
                      <p className={`text-sm font-bold ${isLiveEnded ? 'text-orange-600 dark:text-orange-400' : 'text-slate-600 dark:text-slate-300'}`}>라이브 강제 종료</p>
                      <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mt-0.5">체크 시 자동 감지를 무시하고 라이브 뱃지를 숨깁니다</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={isLiveEnded}
                ...
                <div className="border-t border-slate-100 dark:border-slate-700 pt-8 space-y-4">
                  <div className="flex justify-between items-end px-2">
                    <label className="text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight">
                      참여 멤버 선택 <span className="text-indigo-500 ml-1">({selectedStreamers.length})</span>
                    </label>
                  </div>
                  {editErrors.streamerIds && (
                    <p className="flex items-center gap-1 text-xs font-bold text-red-500 px-2">
                      <AlertCircle className="w-3 h-3 shrink-0" />{editErrors.streamerIds}
                    </p>
                  )}
                  <StreamerSelector streamers={sortedStreamers} selectedStreamers={selectedStreamers} toggleStreamer={toggleStreamer} />
                </div>
              </div>
              ...
              {isHoi4Game && (
                <label className="flex items-center justify-between gap-3 px-4 py-3 bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-800/50 rounded-2xl cursor-pointer">
                  <div>
                    ...
                    <p className="text-xs font-medium text-amber-500 dark:text-amber-600 mt-0.5">체크 시 HOI4 참전 기록 페이지에 집계됩니다</p>
                  </div>
                  <input type="checkbox" checked={isNaeJeon} onChange={(e) => setIsNaeJeon(e.target.checked)} className="w-4 h-4 rounded accent-amber-500 shrink-0" />
                </label>
              )}
              ...
              {isHoi4Game && participants.length > 0 && (
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">HOI4 · 국가</label>
                    ...
                    {participants.map(({ id, nation }) => {
                      ...
                      return (
                        <div key={id} className="flex items-center gap-2 p-2.5 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                          <span className="text-sm font-bold text-slate-700 dark:text-slate-200 w-16 shrink-0 truncate">{streamer.name}</span>
                          <input
                            type="text"
                            value={nation}
            ...
            <div className="p-6 md:p-8 bg-slate-50 dark:bg-slate-800 flex flex-col gap-3 border-t border-slate-100 dark:border-slate-700 shrink-0">
              {editErrors.submit && (
                <p className="flex items-center gap-1.5 text-xs font-bold text-red-500 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-2.5">
                  <AlertCircle className="w-4 h-4 shrink-0" />{editErrors.submit}
                </p>
              )}
              <div className="flex gap-3">
                <button type="button" onClick={onClose} className="flex-1 py-4 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-bold border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">취소</button>
                <button
                  form="schedule-form"
                  type="submit"
                ...
                >
                  {editMetaLoading ? '정보 가져오는 중...' : isSubmitting ? '저장 중...' : isEdit ? '수정 완료' : '일정 등록'}
                </button>
              </div>
            </div>
          </>
        ...
        /* ══════════════════════════════════════════════════════════ */
          <>
                ...
                {slots.map((slot, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => setExpandedKey(isExpanded ? null : slot.key)}
                      ...
                      >
                        ...
                        <span className={`text-sm font-black shrink-0 ${isExpanded ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300'}`}>
                          일정 {index + 1}
                        </span>
                        {!isExpanded && (
                          <span className="text-sm text-slate-500 dark:text-slate-400 truncate flex-1 font-medium">
                            {slot.title ? (
                              <>
                                <span className="font-bold text-slate-700 dark:text-slate-200">{slot.title}</span>
                                {preview && <span className="ml-2 text-slate-400 dark:text-slate-500 text-xs">{preview}</span>}
                              </>
                            ) : (
                              <span className="text-slate-400 dark:text-slate-500 italic">미입력</span>
                            )}
                          </span>
                        )}
                          ...
                          >
                            <div className="px-4 pb-5 pt-1 space-y-4 border-t border-slate-100 dark:border-slate-700">
                              ...
                              <div className="pt-3">
                                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase">
                                  방송 링크
                                  <span className="ml-1.5 normal-case font-medium text-indigo-400 dark:text-indigo-500">· 치지직 URL 자동 채우기</span>
                                </label>
                                <div className="relative">
                                  <LinkIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                  <input
                                    type="text"
                                    value={slot.liveUrl}
                                    onChange={(e) => updateSlot(slot.key, { liveUrl: e.target.value, autoFilled: [] })}
                                    onBlur={() => handleSlotLiveUrlBlur(slot.key, slot.liveUrl, slot.title, slot.selectedGameId, slot.selectedStreamerIds)}
                                    placeholder="https://chzzk.naver.com/live/..."
                                  ...
                                  />
                                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                    {slot.metaLoading && <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />}
                                    {!slot.metaLoading && slot.autoFilled.length > 0 && <Sparkles className="w-4 h-4 text-indigo-400" />}
                                  </div>
                                </div>
                                <AnimatePresence>
                                  {slot.autoFilled.length > 0 && (
                                    ...
                                    >
                                      ...
                                      <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400">자동 입력: {slot.autoFilled.join(' · ')}</p>
                                      <button type="button" onClick={() => updateSlot(slot.key, { autoFilled: [] })} className="ml-auto text-indigo-300 hover:text-indigo-500 transition-colors">
                                        <X className="w-3 h-3" />
                                      </button>
                                    </motion.div>
                                  )}
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  value={slot.title}
                                  onChange={(e) => updateSlot(slot.key, { title: e.target.value, errors: { ...slot.errors, title: undefined } })}
                                  placeholder="예) 문명 6 합방"
                                ...
                                />
                                {slot.errors.title && <p className="mt-1 flex items-center gap-1 text-xs font-bold text-red-500"><AlertCircle className="w-3 h-3 shrink-0" />{slot.errors.title}</p>}
                              </div>
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) => updateSlot(slot.key, { selectedGameId: e.target.value })}
                              ...
                              <div>
                                <div className="flex items-center justify-between mb-1.5">
                                  ...
                                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                                    <input
                                      ...
                                      onChange={(e) => updateSlot(slot.key, { isTimeTBD: e.target.checked })}
                                ...
                                <input
                                  ...
                                  value={slot.isTimeTBD ? slot.startTime.split('T')[0] : slot.startTime}
                                  onChange={(e) => updateSlot(slot.key, { startTime: e.target.value, errors: { ...slot.errors, startTime: undefined } })}
                                ...
                                />
                                {slot.errors.startTime && <p className="mt-1 flex items-center gap-1 text-xs font-bold text-red-500"><AlertCircle className="w-3 h-3 shrink-0" />{slot.errors.startTime}</p>}
                              </div>

                              {/* 참여 멤버 */}
                              <div className="border-t border-slate-100 dark:border-slate-700 pt-4 space-y-3">
                                <label className="text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight">
                                  참여 멤버 <span className="text-indigo-500">({slot.selectedStreamerIds.length})</span>
                                </label>
                                {slot.errors.streamerIds && (
                                  <p className="flex items-center gap-1 text-xs font-bold text-red-500">
                                    <AlertCircle className="w-3 h-3 shrink-0" />{slot.errors.streamerIds}
                                  </p>
                                )}
                                <StreamerSelector
                                  compact
                                  ...
                                  toggleStreamer={(id) => {
                                    ...
                                    updateSlot(slot.key, {
                                      ...
                                      errors: { ...slot.errors, streamerIds: undefined },
                                    });
                                  }}
                                />
                              </div>
            ...
            <div className="p-6 md:p-8 bg-slate-50 dark:bg-slate-800 flex flex-col gap-3 border-t border-slate-100 dark:border-slate-700 shrink-0">
              {batchSubmitError && (
                <p className="flex items-center gap-1.5 text-xs font-bold text-red-500 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-2.5">
                  <AlertCircle className="w-4 h-4 shrink-0" />{batchSubmitError}
                </p>
              )}
              <div className="flex gap-3">
                <button type="button" onClick={onClose} className="flex-1 py-4 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-bold border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">취소</button>
                <button
                  ...
                  className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all disabled:opacity-50"
                >
                  {isSubmitting ? '등록 중...' : `${slots.length}개 일정 등록`}
                </button>
              </div>
            </div>
          </>
