...
export default function StreamerDetailModal({
  ...
  const [showAllLinkedClips, setShowAllLinkedClips] = useState(false);
  const visibleLinkedClips = showAllLinkedClips ? linkedClips : linkedClips.slice(0, 4);
            ...
            {linkedClips.length > 0 && (
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {visibleLinkedClips.map((clip) => (
                    <article
                      key={clip.id}
                      className="group rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 overflow-hidden hover:border-indigo-200 dark:hover:border-indigo-700 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-all"
                    >
                      ...
                      <div className="relative aspect-video bg-slate-100 dark:bg-slate-800">
                        {clip.thumbnailUrl ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-linear-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <a
                          ...
                          className="text-sm font-bold text-slate-700 dark:text-slate-200 line-clamp-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                        >
                          {clip.title}
                        </a>
                        <a href={clip.url} target="_blank" rel="noopener noreferrer" className="shrink-0 mt-0.5">
                          <ExternalLink className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600 group-hover:text-indigo-400 transition-colors" />
                        </a>
                      </div>
                      ...
                      {clip.schedule && (
                        <Link
                          ...
                          className="mt-2 inline-flex items-center gap-1.5 px-2 py-1 rounded-xl text-[11px] font-bold bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <CalendarDays className="w-3 h-3" />
                          <span className="line-clamp-1">{clip.schedule.title}</span>
                        </Link>
                      )}

                      <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
                        {clip.participants.slice(0, 3).map(({ streamer: p }) => (
                          <span
                            key={p.id}
                            className="text-[10px] font-bold px-2 py-0.5 rounded-full border"
                            style={{
                              backgroundColor: `${p.colorCode}20`,
                              borderColor: `${p.colorCode}50`,
                              color: p.colorCode,
                            }}
                          >
                            {p.name}
                        ...
                        {clip.clipDate && (
                          <span className="ml-auto text-[10px] font-bold text-slate-400 dark:text-slate-500">
                            {format(new Date(clip.clipDate), 'yyyy.MM.dd', { locale: ko })}
                          </span>
                        )}
                      </div>
                      </div>
                ...
                {linkedClips.length > 4 && (
                  <button
                    type="button"
                    onClick={() => setShowAllLinkedClips((prev) => !prev)}
                    className="w-full py-2 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 text-xs font-black text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-700 transition-colors"
                  >
